...
def dump_to_s3(args, settings):
    ...
    for source in settings("sources"):
        ...
        if read_access is None:
            ...
            continue
        if not selection.match_schema(source_name):
            logging.info("Skipping source %s which does not match '%s'", source_name, selection.schema)
            continue
        ...
        with closing(etl.pg.connection(etl.env_value(read_access), autocommit=True, readonly=True)) as conn:
            ...
            source_prefix = "{}/{}".format(args.prefix, source_name)
