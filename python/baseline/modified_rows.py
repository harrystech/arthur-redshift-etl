...
"""
Determine how many rows were updated in the last month (or some time period).
"""
...
def find_tables(conn, schemas):
    ...
    tables = etl.pg.query(conn, """SELECT ns.nspname as "schemaname", cls.relname as "tablename"
                                    ...
                                    WHERE ns.nspname in %s
                                      ...
                                      AND (att.attname = 'updated_at' OR att.attname = 'created_at')
...
def find_modified_rows(args, settings):
    ...
    with closing(etl.pg.connection(dw, autocommit=True, readonly=True)) as cx:
        ...
        total_update = count_updated_rows(cx, tables, args.begin_time, args.end_time, check_modified=False)
