    ...
    if "columns" in update_keys:
        ...
        for column in table_design["columns"]:
            if column["name"] in existing_column:
                ...
                update_column_definition(column, existing_column[column["name"]])
...
def update_column_definition(new_column: dict, old_column: dict) -> dict:
    """
    ...
    logger.debug("Trying to update column '%s': new='%s', old='%s'", name, new_sql_type, old_sql_type)
    ...
    if new_sql_type in ("integer", "bigint"):
        if old_sql_type == "bigint" and new_sql_type == "integer":
            ...
            logger.warning("Keeping previous definition of '%s' for column '%s'", new_column["sql_type"], name)
        return new_column
    ...
    if new_text:
        ...
        if old_text:
            [new_size], [old_size] = new_text.groups(), old_text.groups()
            if new_size < old_size:
                new_column["sql_type"] = "character varying({})".format(old_size)
                logger.warning(
                    "Re-using old definition of '%s' for column '%s' as '%s' (please add a cast)",
                    old_column["sql_type"],
                    name,
                    new_column["sql_type"],
                )
